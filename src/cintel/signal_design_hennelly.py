"""
signal_design_hennelly.py - Project script (example).
...
Paths (relative to repo root)
    ...
    INPUT FILE: data/system_metrics_hennelly.csv
    OUTPUT FILE: artifacts/signals_hennelly.csv
...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_hennelly.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "signals_hennelly.csv"
...
def main() -> None:
    ...
    # ----------------------------------------------------------------
    # STEP 2.7 (NEW): DEFINE HIGH ERROR ALERT SIGNAL AT 5% THRESHOLD
    # ----------------------------------------------------------------
    # Flag observations where error_rate exceeds threshold.
    # This is useful for alerting or filtering anomalies.
    ERROR_RATE_THRESHOLD: Final[float] = 0.05  # 5%

    high_error_alert_recipe: pl.Expr = (
        pl.when(pl.col("errors") / pl.col("requests") > ERROR_RATE_THRESHOLD)
        .then(pl.lit(True))
        .otherwise(pl.lit(False))
        .alias("high_error_alert")
    )
    # --------------------------------------------------------------------
    # STEP 2.8: APPLY THE SIGNAL RECIPES TO THE DATAFRAME (OLD STEP 2.7)
    # --------------------------------------------------------------------
    ...
    df_with_signals: pl.DataFrame = df.with_columns(
        [
            ...
            high_error_alert_recipe,  # Added the new alert signal
        ]
    )

    LOG.info(
        "Created signal columns: error_rate, avg_latency_ms, throughput, high_error_alert"
    )  # Added high_error_alert to the log message
    ...
    signals_df = df_with_signals.select(
        [
            ...
            "avg_latency_ms",
            "throughput",
            "high_error_alert",  # Added the new alert signal in the output
        ]
    )
